...
public class StateView extends RelativeLayout {
    ...
    private void init(Context context) {
        ...
        String font = GlobalConfig.getFontFileName();

        errorTV = (EffectTextView) findViewById(R.id.stateview_error_text);
        if (!TextUtils.isEmpty(font)) {
            errorTV.setTypeface(FontManager.getInstance(context).getFont(font));
        }
        errorTV.setEffectType(EffectFactory.TYPE_TYPER);

        actionBtn = (EffectTextView) findViewById(R.id.stateview_action_btn);
        if (!TextUtils.isEmpty(font)) {
            actionBtn.setTypeface(FontManager.getInstance(context).getFont(font));
        }
        actionBtn.setTextColor(GlobalConfig.getHeadBgColor());
    ...
    public void showErrorBecauseNoNetworking() {
        showErrorWithAction("没有网络连接，请设置", "设置", () -> UIUtil.startNetSettingActivity(getContext()));
    }
